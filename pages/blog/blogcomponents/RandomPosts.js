        ...
        {data?.length > 0 ? (
          data.slice(2, 8).map((post, i) => (
            <div key={`key-${i}`} className="lg:px-0  w-full">
              ...
              <p className=" lg:pb-3 hover:text-orange-400 font-bold lg:text-lg text-xs  text-left pt-4 h-10 lg:h-16 lg:block hidden">
                <Link
                  href={`/blog/posts/${post.slug
                    .toLowerCase()
                    .replace(/ /g, "-")}`}
              ...
              <p className="text-left text-[#6B6B6B] lowercase text-xs pt-4 tracking-wide leading-5 lg:hidden block h-24">
                {ParseP(post?.content) && ParseP(post?.content).slice(0, 50)}...
              </p>
              <p className="text-left text-[#6B6B6B] text-base pt-4 tracking-wide lowercase leading-6 lg:block hidden h-28">
