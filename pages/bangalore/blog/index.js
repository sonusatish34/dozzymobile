...
import PostsListing from '@/pages/blog/blogcomponents/PostsListing';
import BlogLayout from '@/pages/blog/blogcomponents/BlogLayout';
import RandomPosts from '@/pages/blog/blogcomponents/RandomPosts';
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const postsQueryBng = query(collection(fireDb, "blogPost"),
          where("blog_state", "==", "active"),
          where("blogfor", "==", "DozzyBng")
        );
        const postsQuerySnapshotBng = await getDocs(postsQueryBng);
        const postsBng = postsQuerySnapshotBng.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const sortedPostsBng = postsBng.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        const all = [...sortedPostsBng,...sortedPosts]
        const allSortedPosts = all;
        
        
        setPostlist(allSortedPosts);
        setSortedPostlist(allSortedPosts);
      } catch (err) {
        ...
        <BlogLayout catg={"Blog"}>
          <div className='xl:px-32 lg:px-12 flex flex-col items-center helvetica-font text-black'>
            ...
            <div className='text-center flex justify-center lg:pt-10 pt-4'>
              <PostsListing loc={'bangalore'} data={sortedPostlist} />
            </div>
            <RandomPosts loc={'bangalore'} data={postlist} />
          </div>
          <div className=" py-7  lg:py-5 flex flex-row xl:pl-36 lg:pl-20 pl-3 helvetica-font">
            <Link href={`/bangalore/blog/${'party'}`} className="flex space-x-2">
...
export async function getServerSideProps({ req, query, params }) {
  ...
  const canonicalUrl = host.includes('.in')
    ? `https://www.dozzy.in/bangalore/blog`
    : `https://www.dozzy.com/bangalore/blog`;
