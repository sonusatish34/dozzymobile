...
const FarmProductLPage = ({ count, FHList }) => {
    ...
    // Intersection Observer to detect if slide is in viewport
    const isInViewport = (entry) => {
                                                ...
                                                <div className="bg-red-2 rounded-lg shadow-lg overflow-hidden flex justify-center flex-col xl:w-[360px] lg:w-[260px] w-[100%] px-1 md:w-80 h-[680px] ">
                                                    ...
                                                    <div className="px- flex flex-col justify-center gap-4">
                                                        <div className="flex items-baseline justify-between px-2 py-2">
                                                            <div>
                                                                <Link
                                                                    className="text-[#556EE6] xl:text-2xl lg:text-lg text-base font-semibold uppercase"
                                                                    href={`/${item.property_name.toLowerCase().replaceAll(/_/g, " ").replace(/\d+/g, ' ').replace('dozzy', '').trim().replaceAll(/ /g, '-')}`}
                                                                >
