...
const FAQComponent = ({ city }) => {
    const faqData = [
        {
            question: 'What is Dozzy and how does it help with farmhouse rentals in Bangalore?',
        ...
        {
            question: 'What amenities do Dozzy’s rental farmhouses in Bangalore include?',
        ...
        {
            question: 'Are there discounts available on farmhouse rentals in Bangalore?',
        ...
        <div className="xl:mx-28 lg:mx-16 ">
            {faqData.map((item, index) => (
                ...
                <div key={index} className="border-b-[2px] border-gray-300 rounded mb-2 text-black">
                    ...
                    >
                        <span className="lg:w-full lg:text-lg font-semibold capitalize mxs:text-base text-sm pr-4 lg:pr-0">{item.question}</span>
                        <svg
                            className={`lg:w-6 lg:h-6 w-6 h-4 rounded bg-[#660066] text-white transition-transform ${activeIndex === index ? 'transform rotate-180' : ''}`}
